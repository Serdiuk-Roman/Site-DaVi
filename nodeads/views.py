...
def owm(request):
    ...
    for i in forecast['list']:
        ...
        if not x:
            ...
            c.lon = float(forecast['city']['coord']['lon'])
            c.lat = float(forecast['city']['coord']['lat'])
            c.temp_max = float(i['main']['temp_max'])
            c.temp_min = float(i['main']['temp_min'])
            c.wind = float(i['wind']['speed'])
            c.cloud = int(i['clouds']['all'])
            c.pressure = float(i['main']['pressure'])
